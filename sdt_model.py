...
class MaskedKLDivLoss(nn.Module):
    ...
    def forward(self, log_pred, target, mask):
        mask_ = mask.view(-1).to(torch.bool)
        loss = self.loss(log_pred, target)[mask_].sum() / torch.sum(mask + 1e-7)
        return loss
...
class MaskedNLLLoss(nn.Module):
    ...
    def forward(self, pred, target, mask):
        ...
        if type(self.weight) == type(None):
            loss = self.loss(pred, target)[mask_].sum() / torch.sum(mask + 1e-7)
        else:
            loss = self.loss(pred, target)[mask_].sum() \
